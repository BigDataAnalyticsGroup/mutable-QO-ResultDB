...
def generate_join_graph_components(input_file:str, mutable:bool = False) -> JGComponents:
    with (open(input_file, 'r') as f):
        ...
        from_clause = from_clause.replace("from", "")
        relation_blocks = from_clause.split(",")
        alias_to_relation = dict()
        for block in relation_blocks:
            split_block = block.split(" ")
            assert len(split_block) <= 3, 'oh no'
            if len(split_block) == 2:
                alias_to_relation[split_block[1]] = split_block[1]
            else:
                alias_to_relation[split_block[2]] = split_block[1]
        # Create a dictionary with alias as keys and column names as values
        projections: dict[str, list[str]] = defaultdict(list)
        for alias in alias_to_relation:
            projections[alias] = ["d", "s"]
            attributes[alias] = {"d", "s"}
...
def parse_sql_to_join_graph(input_file: str):
        ...
        query_definition = f"def create_{query_name}():\n"
        ...
        with open('./benchmark/result-db-eval/ce/query_definitions.py', 'a') as out:
            out.write(query_definition)


if __name__ == '__main__':
    # Command Line Arguments
    parser = argparse.ArgumentParser(prog = 'Generate CE query graphs',
                                     description = '''Script to generate CE query graphs.''')
    ...
    for root, dirs, files in os.walk("./benchmark/result-db-eval/ce/cycle-queries/"):
        for file in files:
            if not "watdiv_cyclic_q9" in file:
                continue
            query_name = file.split(".")[0].replace("watdiv_cyclic_", "")
            print(query_name)
            if config['mutable']:
                parse_sql_to_mutable_query(os.path.join(root, file))
            else:
                parse_sql_to_join_graph(os.path.join(root, file))
