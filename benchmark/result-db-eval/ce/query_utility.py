...
class Relation:
    ...
    def get_query_for_reduced_relation(self):
        query_filters = " WHERE " + ' AND '.join([f.replace('"', "'") for f in self.filters]) if (len(self.filters) > 0) else ''
        return f"SELECT DISTINCT * FROM {self.name} AS {self.alias} {query_filters}"
