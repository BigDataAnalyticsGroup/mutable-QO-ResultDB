...
FROM
	...
	title AS t2
WHERE
	cn1.country_code != "[us]" AND
	it1.info = "rating" AND
	it2.info = "rating" AND
	(kt1.kind = "tv series" OR kt1.kind = "episode") AND
	(kt2.kind = "tv series" OR kt2.kind = "episode") AND
	(lt.link = "sequel" OR lt.link = "follows" OR lt.link = "followed by") AND
	mi_idx2.info < "3.5" AND
	t2.production_year >= 2000 AND
	t2.production_year <= 2010 AND
	lt.id = ml.link_type_id AND
	t1.id = ml.movie_id AND
	it1.id = mi_idx1.info_type_id AND
	t1.id = mi_idx1.movie_id AND
	kt1.id = t1.kind_id AND
	cn1.id = mc1.company_id AND
	t1.id = mc1.movie_id AND
	it2.id = mi_idx2.info_type_id AND
	t2.id = mi_idx2.movie_id AND
	kt2.id = t2.kind_id AND
	cn2.id = mc2.company_id AND
	ml.linked_movie_id = mi_idx2.movie_id AND
