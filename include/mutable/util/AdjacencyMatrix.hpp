...
namespace m
{
    ...
    public:
         ...
         * `src`. */
        SmallBitset reachable(SmallBitset src, SmallBitset S, SmallBitset X = SmallBitset(0)) const
        {
            SmallBitset R_old(0);
            S = S - X; // exclude nodes from X
        ...
        bool is_connected_after_removal(SmallBitset removed, SmallBitset block) const
        {
            ...
            auto rec = [&](SmallBitset node, auto &&rec) -> void
            {
                ...
                for (size_t neighbor_id : allowed_neighbors)
                {
                    ...
                    if (not(visited & neighbor).empty())
                        continue;
                    rec(neighbor, rec);
                }
            };
            /* Take any node from the remaining nodes */
            const auto root = block - removed;
            if (root.empty())
            {
                return true;
            }
        ...
        [[nodiscard]] std::vector<SmallBitset> find_two_vertex_cuts(const SmallBitset block, const SmallBitset problem) const {
            std::unordered_set<SmallBitset, SmallBitsetHash> visited_pairs;
            std::vector<SmallBitset> vertex_cuts;
            auto rec = [&](const SmallBitset node, auto &&rec) -> void
            {
                for (const std::size_t neighbor_id : problem & neighbors(node))
                {
                    auto neighbor = SmallBitset::Singleton(neighbor_id);
                    auto pair = node | neighbor;
                    if (visited_pairs.contains(pair))
                        continue;
                    if (not is_connected_after_removal(pair, block))
                        vertex_cuts.emplace_back(pair);
                    visited_pairs.emplace(pair);
                    rec(neighbor, rec);
                }
            };
            /* Take any node from the remaining nodes */
            const SmallBitset root = block;
            if (root.empty())
            {
                return {};
            }
        ...
        void compute_blocks_and_cut_vertices(std::vector<SmallBitset> &blocks, SmallBitset &cut_vertices, const SmallBitset limit, const std::size_t min_size = 2) const {
            ...
            auto rec = [&](SmallBitset current, std::size_t current_depth, auto &&rec) -> SmallBitset
            {
                ...
                /* Check all neighbors */
                for (const auto neighbor_id : neighbors(current) & limit)
                {
                    /* Neighbor was not explored yet */
                    if (SmallBitset neighbor = SmallBitset::Singleton(neighbor_id); not visited.contains(neighbor))
                    {
                        ...
                        if (low[neighbor] >= depth[current])
                        {
                            ...
                            if (child_block.size() >= min_size)
                            {
                                ...
                                if (current != limit.hi() or child_count > 1)
                                    cut_vertices |= current;
                            }
                        }
                        else
                        {
        ...
        {
            if (this->num_vertices_ != other.num_vertices_)
                return false;
