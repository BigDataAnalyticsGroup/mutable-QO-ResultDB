...
namespace m
{
    ...
    public:
        explicit TreeEnumerator(const std::size_t n) : cost_table_(n), model_table_(n) {}
        std::pair<std::size_t, double> find_best_root(QueryGraph &G, std::unordered_set<std::size_t> &required_reductions, AdjacencyMatrix &adj_matrix, const CardinalityEstimator &CE, SemiJoinCostFunction &SC, card_order_t card_orders[], std::vector<std::unique_ptr<DataModel>> &base_models);
        void update_cost_table(const std::size_t parent, const std::size_t child, double costs, bool needs_reduction)
        {
            cost_table_[parent].emplace(child, std::pair(costs, needs_reduction));
        }

        void compute_cardinality_order(const QueryGraph &G, AdjacencyMatrix &adj_matrix, const CardinalityEstimator &CE, std::size_t node, std::vector<std::size_t> &card_order, std::vector<std::unique_ptr<DataModel>> &base_models);

        void determine_reduced_models(QueryGraph &G, AdjacencyMatrix &adj_matrix, const CardinalityEstimator &CE, card_order_t card_orders[], std::vector<std::unique_ptr<DataModel>> &base_models);

        void update_model_table(const std::size_t parent, const std::size_t child, std::unique_ptr<DataModel> model)
        {
            model_table_[parent].emplace(child, std::move(model));
        }

        auto parent_node_costs(const std::size_t parent, const std::size_t child)
        {
            return cost_table_[parent].find(child);
        }

        auto parent_node_model(const std::size_t parent, const std::size_t child)
        {
            return model_table_[parent].find(child);
        }

        [[nodiscard]] auto cost_begin(const std::size_t parent) const { return cost_table_[parent].begin(); }
        [[nodiscard]] auto model_begin(const std::size_t parent) const { return model_table_[parent].begin(); }
        [[nodiscard]] auto cost_end(const std::size_t parent) const { return cost_table_[parent].end(); }
        [[nodiscard]] auto model_end(const std::size_t parent) const { return model_table_[parent].end(); }
    };
    ...
    namespace Optimizer_ResultDB_utils
    {
        ...
        using folding_problem_t = std::pair<Subproblem, Subproblem>; // (Problem, Block)
        using problem_set_t = std::vector<folding_problem_t>;
        using tree_problem_t = std::vector<problem_set_t>;
        using folding_table_entry_t = std::shared_ptr<std::pair<std::vector<Subproblem>, double>>;
        using folding_table_t = std::unordered_map<Subproblem, folding_table_entry_t, SubproblemHash>;
        ...
        std::pair<std::vector<semi_join_order_t>, double> enumerate_semi_join_reduction_order(QueryGraph &G, std::unordered_set<std::size_t> required_reductions, AdjacencyMatrix &adj_matrix, const CardinalityEstimator &CE, std::vector<std::unique_ptr<DataModel>> &base_models);
        ...
        std::vector<tree_problem_t> create_tree_sets(bc_forest_t &bc_forest, const std::vector<Subproblem> &blocks, Subproblem removed_cut_vertex);
...
;
        ...
        void create_folded_adjacency_matrix(const std::vector<Subproblem> &folds, AdjacencyMatrix &old_matrix, AdjacencyMatrix &new_matrix, std::unordered_map<Subproblem, Subproblem, SubproblemHash> &folded_mapping);
